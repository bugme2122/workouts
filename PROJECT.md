...
| `node --test` (client) / `vitest` + `supertest` + `mongodb-memory-server` (server) | Two suites, 137 + 61 tests (run on push/PR by `.github/workflows/test.yml`) | Client keeps zero dependencies; the server tests run against an in-memory MongoDB, so no Atlas access is needed to run them. |
...
**Surfaces.** Welcome, landing, home and customize paint `body.surface-light` or `body.surface-dark` according to `config.surface` (`system` follows `prefers-color-scheme`; `light`/`dark` override it), decided in `applySurface()` from the pure `resolveSurface()`. The **live timer is always dark** and adds `body.on-live`, which restores its original near-black ground, radial wash and film grain — the one screen whose look predates the surfaces. The dark palette is derived from the light one (same roles, hue-biased neutrals), not from the old chrome; the interval strip's steel/rust marks are re-stepped per ground (ΔE 25.2 protan on white, 22.8 on dark). One shared **settings sheet** serves two masters: opened from a gear button it edits `config` directly (Apply = sanitize + persist + rebuild); opened from the customize screen (`sheetFromCustomize = true`) it edits the customize `draft` and Apply just closes back to customize. The `draft` variable is shared between the sheet and the customize screen — that flag is the only thing distinguishing the flows.
...
2. **Appearance is a config field, not a CSS media query.** `config.surface` is sanitized against a whitelist, synced like the rest of the config, and excluded from `LIGHT_FIELDS` so a share link can't repaint someone else's app. The OS is consulted only when it says `system`, via a `matchMedia` listener that re-paints on change.
3. **Class names collide across the two worlds.** `styles.css` carries the dark app's unscoped rules — `button.ghost` and `.count` (the timer's countdown overlay) among them — so landing styles are namespaced `lp*` and its buttons use `.lpbtn.lpghost`, not `.ghost`. Check for an existing selector before adding a generic class name.
4. **`styles.css` is the only stylesheet.** The never-linked `design-tokens.css`, which duplicated the token block, was deleted (GAPS #11). Themes are applied by JS `style.setProperty`, not a data attribute.
5. **Themes are two different mechanisms:** the 4 phase colors come from `THEMES` in catalog.js applied via `applyTheme()`; the 6 per-person colors (`--p1..--p6`) are fixed in styles.css and do not change with theme.
6. **`sanitize` mutates in place.** Callers that need the original clone first (`sanitize(clone(draft))` is the idiom).
...
8. **A share link opened by an existing user is ignored completely** — `decideBoot()` returns `configSource: "local"`, so the hash is stripped, welcome is shown, and `loadConfig()` never reads the link. (It used to half-apply; see GAPS #5.)
9. **rAF timer in a backgrounded tab:** the loop stops getting frames. On return, `advancePhases()` fast-forwards with exact wall-clock accounting and announces **only the phase landed on** (the phases flown past come back as `skipped` and are deliberately silent), and the `visibilitychange` handler re-renders immediately.
10. **There is no self-registration.** A new environment has no users until one is provisioned — `cd server && npm run create-admin`, or just `npm run dev:local`, which seeds `admin@local.test` / `LocalAdmin!2026` on first run. If sign-in "breaks" in a fresh environment, check that a user exists before suspecting the code.
11. **`docs/superpowers/`** contains the design specs and implementation plans (brainstorm → spec → plan workflow) for the catalog, audio cues, share links, and accounts features. They explain most "why"s; the accounts spec documents locked-in decisions. `docs/agents/prompt.md` is an orchestration prompt template for driving the `.claude/agents/*` subagent pipeline — not app documentation.
